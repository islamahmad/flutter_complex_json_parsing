              ...
              itemBuilder: (BuildContext context,int position){
                return Column(
                  children: <Widget>[
                    ...
                    ListTile(
                        title: Text(data[position]['name'],style: TextStyle(fontSize: 15.9),
                        ),
                        subtitle: Text(data[position]['email'],style: TextStyle(fontSize: 10.9)
                        ),
                        leading: CircleAvatar(child: Text(data[position]['id'].toString()
                        ),
                        ),
                        onTap: () {
                          showOnTapDB(context, data[position]['address']['street']);
                        }
                    ),
                  ],
                );
...
void showOnTapDB(BuildContext context,  String message){
  var alert = AlertDialog(
    title: Text('Lives in this street'),
